...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        p1 = rg.Point(intersection_center.x - (letter_thickness / 2), intersection_center.y - (letter_thickness / 2))
        p2 = rg.Point(p1.x + letter_thickness, p1.y + height)
        p3 = rg.Point(intersection_center.x - (width / 2), intersection_center.y - (letter_thickness / 2))
        p4 = rg.Point(p3.x + width, p3.y + letter_thickness)
    ...
    def clone(self):
        ...
        #     clone.pdf.
        # --------------------------------------------------------------
        new_capitalt = CapitalT(self.h_rect.get_center(), self.h_rect.get_width(), self.v_rect.get_height(),
                                self.v_rect.get_width())
        new_capitalt.set_colors(self.h_rect.fill_color, self.h_rect.outline_color)
        return new_capitalt
